...
def display_progress():
    ...
    st.markdown(f"**진행: {current} / {total} 시행**")
...
def display_results():
    ...
    st.download_button(
        label="JSON으로 다운로드",
        data=json_data,
        file_name=f"igt_result_{session.session_id}.json",
    ...
    st.download_button(
        ...
        data=csv_content,
        file_name=f"igt_result_{session.session_id}.csv",
        mime="text/csv"
    )
...
def main():
    ...
    if not st.session_state.game_started:
        ...
        with st.sidebar:
            st.markdown("### 게임 정보")
            st.markdown(f"**참가자:** {st.session_state.session.participant_id}")
            st.markdown(f"**세션:** {st.session_state.session.session_id}")

            st.markdown("### 덱 선택 횟수")
            counts = st.session_state.deck_manager.get_deck_counts()
            for deck, count in counts.items():
                st.markdown(f"Deck {deck}: {count}회")
